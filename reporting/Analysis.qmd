...
# Aim 2_1:
## Aim 2a_2: Pre-transplant ferritin status
```{r aim_2a_2}
source("../code/EDA.R")
...
## Aim 2b_2: Post-transplant ferritin status
```{r aim_2b_2}
aim_2b_2 <- cohort_covars %>% filter(aim == "aim_2b_2") %>%
  mutate(death = if_else(death == TRUE, "Death", "Alive"), 
        bacteremia = if_else(is.na(bacteremia), "Not Detected", bacteremia), 
        disease_relapse_days_since_ce = as.numeric(difftime(disease_relapse_date, transplant_date, units = "days")))
...
cr_data <- read.csv("../redcap/IronOverloadChartRev_DATA_2024-05-02_1145.csv") %>% 
           ...
           filter(!grepl("^[A-Za-z]", record_id))
total_ct <- cr_data %>% distinct_ct("record_id")
...
completed_ct <- cr_data %>% distinct_ct("record_id")
true_positives_ct <- cr_data %>% filter(!is.na(transplant_date)) %>% distinct_ct("record_id")
false_positives_ct <- cr_data %>% filter(is.na(transplant_date)) %>% distinct_ct("record_id")
```

- Total number of chart reviews: `r total_ct`
- Number of completed chart reviews: `r completed_ct`
- Number of false positives: `r false_positives_ct`
- Number of true positives: `r true_positives_ct`

```{r cr_status_by_site}
cr_data %>% group_by(site, eligibility) %>% 
      summarise(n = n_distinct(record_id)) %>% ungroup() %>%
      mutate(eligibility = if_else(eligibility == 0, "false_positives", "true_positives")) %>%
