...
label(cohort_covars$last_in_person_visit_since_ce) = "Follow-up duration (yrs)"
...
table1(~gender + age_at_ce + age_group + ethnicity + transplant_type + scd_type + transplant_type_combined + site| aim, 
      data = cohort_t1,
      overall = FALSE, 
...
table1(~no_LIC_pre + scd_type + 
      ...
      transplant_type_combined + site| LIC_1_level, 
      data = aim_2a_1,
...
aim_2a_1_binary <- cohort_covars_LIC %>% filter(!is.na(LIC_level_1_binary), 
                                    ...
                                    chart_completion) %>% 
                                    collect() %>% 
                                    mutate(last_in_person_visit_since_ce = last_in_person_visit_since_ce/365.25) %>%
                                    site_mask() %>%
                                    mutate(site = relevel(as.factor(site), ref = "site_1"))
...
table1(~no_LIC_pre + scd_type + age_at_ce + age_group + gender + 
      ...
      busulfan_and_LIC_over_10 + 
      busulfan_and_VOD_and_LIC_over_10 + 
      transplant_type_combined + site| LIC_level_1_binary, 
...
table1(~no_LIC_pre + scd_type + age_at_ce + age_group + gender + 
      ...
      busulfan_and_LIC_over_10 + 
      busulfan_and_VOD_and_LIC_over_10 + 
      transplant_type_combined + site| LIC_level_3_max_binary, 
...
table1(~no_ferritin_pre + scd_type + age_at_ce + age_group + gender + disease_relapse + 
       ...
       busulfan_and_vod + transplant_type_combined + site| ferritin_pre, 
       data = aim_2a_2, 
       overall = c(right = "Total"), 
...
table1(~no_ferritin_pre + scd_type + age_at_ce + age_group + gender + disease_relapse + 
       ...
       donor_relation + match_status + 
       second_transplant + scd_type + 
       round(last_in_person_visit_since_ce/365.25, 1) + 
       GVHD + vod_subgroup + bacteremia +
       conditioning_type + has_busulfan + 
       busulfan_and_vod + transplant_type_combined + site| ferritin_pre, 
       data = aim_2a_2 %>% 
...
- <span style="background-color: #FFFF00">This table only has patients with complete chart reviews AND verifiable LIC unit conversions </span
```{r aim_2b_1_LIC_post}
aim_2b_1 <- cohort_covars_LIC %>% 
          filter(!is.na(LIC_level), LIC_type == "post-transplant", chart_completion) %>% 
          collect() %>%
          site_mask() %>%
          mutate(site = relevel(as.factor(site), ref = "site_1"))
...
table1(~no_LIC_post + scd_type + age_at_ce + gender + 
      ...
      #  CD3_reconstitution + CD4_reconstitution + CD8_reconstitution + IgM_reconstitution + 
       vod + GVHD + 
      # death_days_since_last_transplant +
      bacteremia + transplant_type_combined + site| LIC_1_level, 
      data = aim_2b_1,
...
aim_2b_2 <- cohort_covars_ferritin %>% 
              filter(chart_completion, !is.na(ferritin_post)) %>% 
              collect() %>%
              site_mask() %>%
              mutate(site = relevel(as.factor(site), ref = "site_1"))
...
table1(~no_ferritin_post + scd_type + age_at_ce + gender + 
      ...
      #  CD3_reconstitution + CD4_reconstitution + CD8_reconstitution + IgM_reconstitution + 
       GVHD + vod_subgroup + 
      # death_days_since_last_transplant +
      bacteremia + transplant_type_combined + site| ferritin_post, 
      data = aim_2b_2,
...
### Ferritin trajectories after transplant
```{r ferritin_reduction}
ferritin_reduction <- results_tbl("ferritin_trajectories_IRT_no_time_constraint") %>% collect() %>%
  mutate(ferritin_date = as.numeric(ferritin_date - transplant_date)) %>%
      mutate(start_date = as.numeric(start_date - transplant_date)) %>%
      mutate(end_date = as.numeric(end_date - transplant_date)) %>% 
      filter(!(person_id %in% c(4502732, 599793, 9720611))) %>%  #remove patients with missing ferritin values
