...
```{r aim_2a_2}
aim_2a_2 <- cohort_covars %>% filter(aim_2a_2, chart_completion) %>% collect()
...
- <span style="background-color: #FFFF00">This table only has patients with complete chart reviews AND verifiable LIC unit conversions </span
```{r aim_2b_1}
aim_2b_1 <- cohort_covars %>% filter(aim_2b_1, !is.na(LIC_level), LIC_type == "post-transplant", chart_completion) %>% collect() 

table1(~disease_relapse + disease_relapse_days_since_ce + death_days_since_ce + 
       LIC_days_since_transplant +
       survival + 
       survival_1yr + survival_3yr + survival_5yr + 
...
- <span style="background-color: #FFFF00">This table only has patients with complete chart reviews </span>
```{r aim_2b_2}
aim_2b_2 <- cohort_covars %>% filter(aim_2b_2, chart_completion) %>% collect() 

table1(~disease_relapse + 
       disease_relapse_days_since_ce + 
       ferritin_days_post + 
       survival + 
       death_days_since_ce + 
...
```{r aim_3}
aim_3 <- cohort_covars %>% filter(aim_3_2 | aim_3_1) %>%
          filter(!is.na(ferritin_post)) %>%
          mutate(overhead = "Post-transplant ferritin levels")
label(aim_3$IRT) = "Post-transplant iron reduction therapy"
table1(~IRT| overhead*ferritin_post, 
      data = aim_3,
      # caption = "Post-transplant ferritin levels",
      overall = FALSE)
```
