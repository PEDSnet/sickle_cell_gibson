...
#' @md
req_basename <- "sickle_cell_Gibson"
    ...
    base_dir <-
      {
       preferred <- Sys.getenv('PEDSNET_DATA_REQUEST_ROOT',
        ...
        if (file.exists(file.path(preferred, 'site', 'run_5P5.R'))) {
          preferred
        }
        else {
            if (any(file.exists(c('run_5P5.R',
                                  file.path('..', 'site', 'run_5P5.R'))))) {
              file.path(getwd()) 
            } else {
              if (file.exists(file.path('site', 'run_5P5.R'))) {
                getwd()
              } else {
                stop("Can't find package base directory; ",
                     "set base_dir in site/run_5P5.R")
              }
            }
          }
      }
...
#' @md
config('results_schema', "v55_sickle_cell_gibson")
